...
import { DataValues, UpdateClubCategoryProps } from './UpdateClubCategory.types'
...
const UpdateClubCategory = ({
    handleClose,
    handleUpdate,
    defaultValues
}: UpdateClubCategoryProps) => {
    const {
        register,
        handleSubmit,
        watch,
    ...
    const onSubmit: SubmitHandler<DataValues> = async (data) => {
        ...
        console.log('data',data.file.length === 1)
        ...
        handleUpdate(defaultValues.id, {name: data.name, image:url})
        handleClose()
    }
            ...
            >
                ...
                <Box sx={{width:"100%"}}>
                    <Button
                        ...
                        <VisuallyHiddenInput
                            type="file"
                            {...register("file", { required: defaultValues?.image ? false : "Este campo es obligatorio"})}
                        />
                    </Button>
                    {errors.file && 
...
export default UpdateClubCategory;